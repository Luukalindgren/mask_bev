...
class Mask2FormerHead(MaskFormerHead):
    ...
    def __init__(self,
        ...
        if self.predict_height:
            self.height_embed = nn.Linear(feat_channels, 12)
        else:
    ...
    def _get_targets_single(self, cls_score: Tensor, mask_pred: Tensor,
        ...
        if self.predict_height:
            ...
            for i in range(len(pos_inds)):
                idx = sampling_result.pos_assigned_gt_inds[i]
                if idx < num_gt:
    ...
    def loss(self, all_cls_scores, all_mask_preds, gt_labels_list,
        ...
        loss_dict['loss_height'] = losses_heights[-1]
        # loss from other decoder layers
        num_dec_layer = 0
        for loss_cls_i, loss_mask_i, loss_dice_i, loss_height_i in zip(
                losses_cls[:-1], losses_mask[:-1], losses_dice[:-1], losses_heights[:-1]):
            loss_dict[f'd{num_dec_layer}.loss_cls'] = loss_cls_i
            loss_dict[f'd{num_dec_layer}.loss_mask'] = loss_mask_i
            loss_dict[f'd{num_dec_layer}.loss_dice'] = loss_dice_i
            loss_dict[f'd{num_dec_layer}.loss_height'] = loss_height_i
            num_dec_layer += 1
        return loss_dict
    ...
    def _loss_by_feat_single(self, cls_scores: Tensor, mask_preds: Tensor,
        ...
        # Heights
        if self.predict_height:
            heights_pred = heights_pred.flatten(0, 1)
            heights = torch.stack([torch.tensor(h) for h in heights_list_gt], dim=0).flatten(0, 1)

            heights_label = torch.zeros_like(heights)
            heights_label[heights > 0] = (heights[heights > 0] - 1) / 0.2 + 1
            heights_label = heights_label.long().to(heights_pred.device)

            loss_height = self.loss_height(heights_pred, heights_label)
        else:
            loss_height = 0
    ...
    def forward(self, x: List[Tensor],
        ...
        return cls_pred_list, mask_pred_list, heights_pred_list
