...
def _kitti_label_to_annos(labels: [KittiLabel]):
    ...
    names = np.array([KittiType.to_string(x.type) for x in labels])
    occluded = np.array([x.occluded for x in labels])
    truncated = np.array([x.truncated for x in labels])
    return dict(name=names, bbox=bboxes, location=locations, dimensions=dimensions, rotation_y=rotation_ys,
                score=scores, alpha=alphas, occluded=occluded, truncated=truncated)
...
def _preds_to_annos(predictions: [Prediction]):
    ...
    names = np.array([KittiType.to_string(x.type) for x in predictions])
    return dict(name=names, bbox=bboxes, location=locations, dimensions=dimensions, rotation_y=rotation_ys,
                score=scores, alpha=alphas)


def eval_kitti(labels: [[KittiLabel]], predictions: [[Prediction]]):
    gt_annos = [_kitti_label_to_annos(x) for x in labels]
    dt_annos = [_preds_to_annos(x) for x in predictions]
    current_classes = [0]
    difficulties = [0, 1, 2]
    z_axis = 1
    z_center = 1.0
    return get_official_eval_result(gt_annos, dt_annos, current_classes, difficulties, z_axis, z_center)


# Inspired by https://github.com/traveller59/kitti-object-eval-python/blob/master/eval.py#L719
def get_mAP(prec):
    sums = 0
